...
func task1(_ optionalString: String?) -> GuardedErrors? {
    ...
    // EDITOR: Remove the remainder of this function and replace it with "return nil"
    guard let aString = optionalString else { return .string_is_nil }
    testPrint("Please type '\(aString)'")
    guard let input = testReadLine() else { return .testReadLine_is_nil }
    guard aString == input else { return .input_does_not_match }
    testPrint("Success!")
    return .success
}
...
func task2() -> Int? {
    ...
    var nameCount = 0
    while true {
        testPrint("Please enter a name or 'done' to stop")
        if let name = testReadLine() {
            if name == "done" { break }
            if name == "" { continue }
...
func task3() -> [task3Func]? {
    ...
    // EDITOR: The following code will be deleted and the /* */ comment above uncommented
    // for tasks.swift. It is used to validate the test code.
    let closureArray: [task3Func] = [
        { // Closure 0:
            first_parameter, second_parameter in
        ...
        { // Closure 3:
            if $0 < 0 { return -1 }
            if $0 >= test3Array.count { return -2 }
            return test3Array[$0] == $1 ? 1 : 0
        },
        { // Closure 4:
            let firstOdd = $0 % 2 == 1
            let secondOdd = $1 % 2 == 1
...
//  Use a for loop and a range to add the numbers from 2 through 7 to returnArray
//  Use a for loop and a stride to add the odd numbers from 1 through 9 to returnArray
//  Use a for loop and a stride to add the numbers from 5 through 0 to returnArray
